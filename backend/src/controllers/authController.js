...
//  Body: { employeeId, password, companyId? }
//  password = last 4 digits of user's registered phone
// ════════════════════════════════════════════════════════════════════════════
exports.userLogin = async (req, res, next) => {
  try {
    const { employeeId, password, companyId } = req.body;
    if (!employeeId?.trim() || !password?.trim()) {
      return res.status(400).json({ message: 'Employee ID and password are required' });
    }

    const query = { employeeId: employeeId.trim(), isActive: true };
    if (companyId) query.companyId = companyId;

    const users = await User.find(query).lean();

    if (users.length === 0) {
      return res.status(404).json({ message: 'No active user found with this Employee ID' });
    }

    // Multiple companies share this employeeId — ask to pick
    if (users.length > 1) {
      ...
      return res.status(300).json({
        ...
        message:   'Multiple companies found for this ID. Please select yours.',
      });
    }
    ...
    // Mark phoneVerified on first login (replaces old OTP verification)
