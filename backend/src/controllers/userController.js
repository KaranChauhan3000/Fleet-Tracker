...
exports.createUser = async (req, res, next) => {
  try {
    ...
    // ── User/driver limit check (same limit as vehicles) ─────────────────
    const membershipDoc = await Company.findById(cid).select('membership').lean();
    const limit   = membershipDoc?.membership?.vehicleLimit || 50;
    const current = await User.countDocuments({ companyId: cid });
    if (current >= limit) {
      return res.status(403).json({
        limitReached: true,
        message: `Driver limit reached (${limit}). Contact Karo India Foundation to increase your limit.`,
        current, limit,
      });
    }
    // ─────────────────────────────────────────────────────────────────────
    ...
    const companyDoc = await Company.findById(cid);
    res.status(201).json({
      id: user._id, name: user.name, employeeId: user.employeeId, phone: user.phone,
      assignedVehicleIds: vIds, isActive: user.isActive, companySlug: companyDoc?.slug,
    });
  } catch (err) { next(err); }
};
