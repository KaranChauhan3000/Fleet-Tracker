...
exports.getStats = async (req, res, next) => {
  try {
    ...
    const [serviceAlerts, pollutionAlerts, insuranceAlerts, challanAlerts, fastagAlerts, finances] = await Promise.all([
      ServiceLog.aggregate([
      ...
      Vehicle.find({ companyId: toOid(cid), fastagBalance: { $ne: null, $lt: 200 } }).select('plateNumber make model fastagBalance fastagStatus fastagLastUpdated'),
    ...
    res.json({
      ...
      fastagAlerts: fastagAlerts.map(v => ({ id: v._id, plateNumber: v.plateNumber, make: v.make, model: v.model, balance: v.fastagBalance, status: v.fastagStatus, lastUpdated: v.fastagLastUpdated })),
