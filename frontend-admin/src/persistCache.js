...
export function pcSet(path, data) {
  try {
    ...
    localStorage.setItem(storageKey(path), JSON.stringify({ data, ts: Date.now(), ttl, _path: path }));
  } catch (e) {
    // localStorage full — prune old entries
    pruneOldest();
    try {
      localStorage.setItem(storageKey(path), JSON.stringify({ data, ts: Date.now(), ttl: getTtl(path), _path: path }));
    } catch { /* silent */ }
  }
}
