...
import {
  api, userApi, saveAuth, saveUserAuth,
  getStoredAdmin, getSavedUserProfile, getUserToken,
...
export default function Login({ onLogin, dark, onToggleTheme, initialStep = 'home', savedProfile = null }) {
  ...
  const storedUser  = getSavedUserProfile();
  ...
  function reset() {
    ...
    setUserPhone(''); setUserPassword(''); setCompanyList([]);
  ...
  // ── Quick resume (stored user) ──────────────────────────────────────────────
  async function handleUserResume() {
    const token = getUserToken();
    if (!token) { clearUserAuth(); setStep('user-login'); return; }
    setLoading(true);
    try {
  ...
  async function handleUserLogin(companyId) {
    if (!userPhone.trim() || !userPassword.trim()) return toast.error('Enter mobile number and password');
  ...
  async function handleRegister() {
    ...
    try {
      ...
      saveAuth(res.token, res.user);
      toast.success(`Welcome to Fleet Pro, ${res.user.name}! Your 30-day trial has started.`);
      onLogin(res.user, true); // true = new registration → show welcome screen
    } catch (e) { toast.error(e.message); } finally { setLoading(false); }
  }
        ...
        <div style={{ width: '100%', maxWidth }}>
          ...
          {storedUser && (
            <Card style={{ marginBottom: 16 }}>
              <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Resume session</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(37,99,235,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <UserCircle2 size={18} style={{ color: '#2563EB' }} />
                </div>
                <div>
                  <p style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)', margin: 0 }}>{storedUser.name}</p>
                  <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0 }}>{storedUser.companyName} · Driver</p>
                </div>
              </div>
              <Btn onClick={handleUserResume} loading={loading} style={{ background: '#2563EB' }}>
                Continue as {storedUser.name.split(' ')[0]}
              </Btn>
            </Card>
          )}
          ...
          <Card>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0 }}>Enter your mobile number & password</p>
              </div>
            </div>
            ...
            <Field label="Mobile Number" icon={Phone}>
              <Input
                type="tel" inputMode="numeric"
                placeholder="e.g. 9876543210"
                value={userPhone} onChange={e => setUserPhone(e.target.value.replace(/\D/g, '').slice(0, 10))}
                disabled={loading}
              />
            </Field>
        ...
        <div style={{ width: '100%', maxWidth }}>
          ...
          <Card>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 18 }}>
              Multiple companies found for <strong>{userPhone}</strong>. Pick yours:
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {companyList.map(c => (
                <button
