...
function DocViewer({ doc, onClose }) {
  ...
  useEffect(() => {
    ...
    function onTouchStart(e) {
      if (e.touches.length === 2) {
        ...
        dragRef.current = {
          startX: e.touches[0].clientX - stateRef.current.offset.x,
          startY: e.touches[0].clientY - stateRef.current.offset.y,
        };
      }
    }
    ...
    function onTouchMove(e) {
      if (e.touches.length === 2 && pinchRef.current) {
        ...
        const sc = Math.max(1, Math.min(5, pinchRef.current.startScale * (dist / pinchRef.current.startDist)));
    ...
    function onTouchEnd(e) {
      ...
      if (e.touches.length === 0) {
        ...
        if (now - lastTap.current < 300) {
          stateRef.current.scale > 1 ? apply(1, 0, 0) : apply(2.5, 0, 0);
        }
        lastTap.current = now;
        dragRef.current = null;
      }
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px 16px 24px', overflow: 'hidden', touchAction: isImage ? 'none' : 'auto' }}>
        {isImage ? (
      ...
      <p style={{ textAlign: 'center', fontSize: 11, color: 'rgba(255,255,255,0.3)', paddingBottom: 16 }}>
        {isImage ? 'Pinch to zoom · Double-tap · Tap outside to close' : 'Tap outside to close'}
      </p>
    </div>
  );
}
