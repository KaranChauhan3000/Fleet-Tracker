...
import {
  ...
  AlertCircle, BadgeIndianRupee, Percent, StickyNote,
} from 'lucide-react';
...
export default function FinanceTracker({ admin, onNavigate, onLogout, dark, onToggleTheme }) {
  ...
  function openEdit(entry) {
    ...
    setForm({
      ...
      totalEmis: String(entry.totalEmis || ''),
      emisPaid: String(entry.emisPaid ?? '0'),
      ...
      <div className="page-content">
        ...
        {totalActive > 0 && (
          <div style={{
            ...
            <p style={{ fontSize: 26, fontWeight: 900, color: '#fff', letterSpacing: '-0.03em', marginTop: 2 }}>
              {fmtRs(totalActiveEmi)}
            </p>
            <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.65)', marginTop: 3 }}>
              {totalActive} active finance{totalActive > 1 ? 's' : ''}
            </p>
          </div>
        )}
        ...
        <Modal title={editEntry ? 'Edit Finance Entry' : 'Add Finance Entry'} onClose={() => setShowForm(false)}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <Field label="Total EMIs" icon={CreditCard}>
              <input style={inputStyle} type="number" placeholder="36" value={form.totalEmis} onChange={e => setForm(f => ({ ...f, totalEmis: e.target.value }))} />
            </Field>
            <Field label="EMIs Already Paid" icon={CheckCircle2}>
              <input style={inputStyle} type="number" placeholder="0" min={0} value={form.emisPaid} onChange={e => setForm(f => ({ ...f, emisPaid: e.target.value }))} />
            </Field>
          </div>
