...
import {
  ...
  AlertTriangle, Plus, User, Calendar, Zap, ChevronLeft, ChevronRight,
  List, Edit2, Trash2, Navigation, IndianRupee, Clock, ShieldAlert, RefreshCw, FileText
} from 'lucide-react';
...
export default function VehicleAnalyticsPage({ vehicleId, onBack, users, toast, fromExpiry, onNavigate }) {
  ...
  const [fastagRefreshing, setFastagRefreshing] = useState(false);
  ...
  async function refreshFastag() {
    setFastagRefreshing(true);
    try {
      const res = await api.patch(`/admin/vehicles/${vehicleId}/fastag`, {});
      setData(prev => prev ? {
        ...prev,
        vehicle: {
          ...prev.vehicle,
          fastagBalance:      res.balance,
          fastagStatus:       res.status,
          fastagLastRecharge: res.lastRecharge,
          fastagCardValidity: res.cardValidity,
          fastagLastUpdated:  res.lastUpdated,
        }
      } : prev);
      toast('FASTag balance updated ✓', 'success');
    } catch (err) {
      toast(err.message || 'FASTag fetch failed', 'error');
    } finally {
      setFastagRefreshing(false);
    }
  }
  ...
  const { vehicle: v, summary: s, monthly, perFill } = data;
            ...
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '13px 14px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 11 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 7, background: 'rgba(251,191,36,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Zap size={13} color="#F59E0B" />
                  </div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>FASTag Balance</p>
                </div>
                <button
                  onClick={refreshFastag}
                  disabled={fastagRefreshing}
                  style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 7, border: '1px solid var(--border)', background: 'var(--bg-elevated)', fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', cursor: fastagRefreshing ? 'not-allowed' : 'pointer', opacity: fastagRefreshing ? 0.6 : 1 }}
                >
                  {fastagRefreshing
              ...
              {v.fastagBalance == null ? (
                <div style={{ textAlign: 'center', padding: '14px 0' }}>
                  <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 4 }}>Balance not fetched yet</p>
                  <p style={{ fontSize: 11, color: 'var(--text-muted)', opacity: 0.6 }}>Tap Refresh to fetch from Surepass</p>
                </div>
              ) : (
                <>
                  {/* Balance + status */}
                  <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 10 }}>
                    <div>
                      {(() => {
                        const bal = v.fastagBalance;
                        const color = bal <= 0 ? '#ef4444' : bal < 200 ? '#f59e0b' : '#22c55e';
                        return (
                          <>
                            <p style={{ fontSize: 28, fontWeight: 900, color, letterSpacing: '-0.02em', lineHeight: 1 }}>₹{bal?.toLocaleString('en-IN') ?? '—'}</p>
                            <p style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 3 }}>
                              {bal <= 0 ? '⚠ Zero balance — blacklist risk' : bal < 200 ? '⚠ Low balance' : 'Sufficient balance'}
                            </p>
                          </>
                        );
                      })()}
                    ...
                    {v.fastagStatus && (
                      <span style={{
                        fontSize: 10, fontWeight: 800, padding: '4px 9px', borderRadius: 7, letterSpacing: '0.04em',
                        background: v.fastagStatus?.toUpperCase() === 'ACTIVE' ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)',
                        color: v.fastagStatus?.toUpperCase() === 'ACTIVE' ? '#22c55e' : '#ef4444',
                        border: `1px solid ${v.fastagStatus?.toUpperCase() === 'ACTIVE' ? 'rgba(34,197,94,0.25)' : 'rgba(239,68,68,0.25)'}`,
                      }}>
                        {v.fastagStatus?.toUpperCase()}
                      </span>
                    )}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {v.fastagLastRecharge && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderRadius: 6, background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
                        ...
                        <span style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600 }}>Last recharge: {v.fastagLastRecharge}</span>
                      </div>
                    )}
                    {v.fastagCardValidity && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderRadius: 6, background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
                        <Clock size={10} color="var(--text-muted)" />
                        <span style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600 }}>Valid till: {v.fastagCardValidity}</span>
                      </div>
                    )}
                    {v.fastagLastUpdated && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderRadius: 6, background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
                        ...
                        <span style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600 }}>
                          Updated {new Date(v.fastagLastUpdated).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>
                    )}
                  </div>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                <MetricCard
                  ...
                  value={s.avgEfficiency != null ? `${s.avgEfficiency} km/L` : '—'}
                  sub={s.efficiencyTrend != null
                    ? <span style={{ color: trendColor, display: 'flex', alignItems: 'center', gap: 3 }}>
                        {TrendIcon && <TrendIcon size={10} />}
                        {Math.abs(s.efficiencyTrend)} km/L vs prev
                      </span>
                    : 'cumulative avg'}
                  icon={<Activity size={13} color="#A78BFA" />}
                  accent="var(--purple-dim)"
                />
                <MetricCard label="Fuel Spend" value={fmtRs(s.recentCost)} sub="last 30 days" icon={<DollarSign size={13} color="var(--success)" />} accent="var(--success-dim)" />
                <MetricCard label="KM Driven" value={s.recentKm > 0 ? `${fmt(s.recentKm, 0)} km` : '—'} sub="last 30 days" icon={<Gauge size={13} color="var(--accent-light)" />} accent="var(--accent-dim)" />
                <MetricCard label="Fuel Logs" value={s.recentFills} sub="entries last 30 days" icon={<Fuel size={13} color="var(--warning)" />} accent="var(--warning-dim)" onClick={() => setTab('logs')} />
              </div>
            </div>
