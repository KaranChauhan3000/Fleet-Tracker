...
import {
  ...
  Navigation, Gauge, Sun, Moon, ChevronLeft, Receipt, Wrench, BarChart2, CreditCard, Share2, ShieldCheck, UserPlus,
} from 'lucide-react';
...
export default function Dashboard({ admin, onNavigate, onLogout, dark, onToggleTheme }) {
  ...
  const totalAlerts = (s.pollutionAlerts?.length||0)+(s.insuranceAlerts?.length||0)+(s.challanAlerts?.length||0)+(s.fastagAlerts?.length||0)+(s.serviceAlerts?.length||0)+(s.emiAlerts?.length||0)+(s.nocAlerts?.length||0);
      ...
      <div className="page-content">
        ...
        {totalAlerts > 0 && (
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:8 }}>
              ...
              <p className="section-title" style={{ margin:0 }}>Alerts</p>
              <span style={{ marginLeft:'auto', fontSize:10, fontWeight:800, color:'var(--accent)', background:'var(--accent-dim)', padding:'2px 8px', borderRadius:10, border:'1px solid rgba(249,115,22,0.2)' }}>{totalAlerts}</span>
            </div>

            {s.pollutionAlerts?.length > 0 && (
              <div style={{ marginBottom:6 }}>
                <p style={{ fontSize:9, fontWeight:800, color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:4, paddingLeft:2 }}>PUC · {s.pollutionAlerts.length}</p>
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  {s.pollutionAlerts.map(a => <ExpiryAlertCard key={a.id} alert={a} onNavigate={onNavigate} type="PUC"/>)}
                </div>
              </div>
            )}
            {s.insuranceAlerts?.length > 0 && (
              <div style={{ marginBottom:6 }}>
                <p style={{ fontSize:9, fontWeight:800, color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:4, paddingLeft:2 }}>Insurance · {s.insuranceAlerts.length}</p>
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  {s.insuranceAlerts.map(a => <ExpiryAlertCard key={a.id} alert={a} onNavigate={onNavigate} type="INS"/>)}
                </div>
              </div>
            )}
            {s.challanAlerts?.length > 0 && (
              <div style={{ marginBottom:6 }}>
                <p style={{ fontSize:9, fontWeight:800, color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:4, paddingLeft:2 }}>Challans · {s.challanAlerts.length}</p>
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  {s.challanAlerts.map(a => <ChallanAlertCard key={a.id} alert={a} onNavigate={onNavigate}/>)}
                </div>
              </div>
            )}
            {s.fastagAlerts?.length > 0 && (
              <div style={{ marginBottom:2 }}>
                <p style={{ fontSize:9, fontWeight:800, color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:4, paddingLeft:2 }}>FASTag Low Balance · {s.fastagAlerts.length}</p>
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  {s.fastagAlerts.map(alert => {
                    const isZero = alert.balance <= 0;
                    const color  = isZero ? 'var(--danger)' : 'var(--warning)';
                    const bg     = isZero ? 'var(--danger-dim)' : 'var(--warning-dim)';
                    const bdrClr = isZero ? 'rgba(220,38,38,0.25)' : 'rgba(217,119,6,0.20)';
                    return (
                      <div key={alert.id} onClick={() => onNavigate('vehicleAnalytics', alert.id)}
                        style={{ padding:'9px 11px 8px', borderRadius:10, cursor:'pointer', border:`1px solid ${bdrClr}`, background:bg, transition:'transform 0.12s', userSelect:'none' }}
                        onMouseDown={e=>e.currentTarget.style.transform='scale(0.98)'}
                        onMouseUp={e=>e.currentTarget.style.transform='scale(1)'}
                      >
                        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                          <span style={{ fontWeight:800, fontSize:12, fontFamily:'var(--font-mono)', color, flex:1 }}>{alert.plateNumber}</span>
                          <span style={{ fontSize:9, fontWeight:700, color:'var(--text-muted)' }}>{alert.make} {alert.model}</span>
                          <span style={{ fontSize:9, fontWeight:800, padding:'2px 7px', borderRadius:5, background:`${color}18`, color, border:`1px solid ${bdrClr}`, letterSpacing:'0.05em' }}>
                            ₹{alert.balance?.toLocaleString('en-IN')??'0'}
                          </span>
                        </div>
                      </div>
                    );
            ...
            {s.serviceAlerts?.length > 0 && (
              <div style={{ marginBottom:2 }}>
                <p style={{ fontSize:9, fontWeight:800, color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:4, paddingLeft:2 }}>Service Due · {s.serviceAlerts.length}</p>
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
            ...
            {s.emiAlerts?.length > 0 && (
              <div style={{ marginBottom:2 }}>
                <p style={{ fontSize:9, fontWeight:800, color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:4, paddingLeft:2 }}>EMI Due · {s.emiAlerts.length}</p>
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  {s.emiAlerts.map(alert => {
                    ...
                    const bg     = isCritical ? 'var(--danger-dim)' : isWarning ? 'var(--warning-dim)' : 'var(--success-dim)';
                    const bdrClr = isCritical ? 'rgba(220,38,38,0.25)' : isWarning ? 'rgba(217,119,6,0.20)' : 'rgba(22,163,74,0.20)';
                    return (
                      <div key={alert.id}
            ...
            {s.nocAlerts?.length > 0 && (
              <div style={{ marginBottom:2 }}>
                <p style={{ fontSize:9, fontWeight:800, color:'var(--text-secondary)', textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:4, paddingLeft:2 }}>NOC Required · {s.nocAlerts.length}</p>
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  {s.nocAlerts.map(alert => {
                    const isAllPaid  = alert.allPaid;
                    const isExpired  = alert.daysToEnd < 0;
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:9 }}>
          <div className="stat-card" style={{ cursor:'default' }}>
            ...
            <p style={{ fontSize:18, fontWeight:800, color:'var(--warning)', marginTop:4 }}>
