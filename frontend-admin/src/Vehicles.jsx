...
export default function VehiclesPage({ admin, onLogout }) {
  ...
  async function load(p = page, q = search) {
    ...
    if (cached) {
      ...
      setLoading(false);
      api.get(path).then(res => { setVehicles(res.data || []); setTotal(res.total || 0); }).catch(() => {});
      return;
    }
    setLoading(true);
    try {
  ...
  if (analyticsVehicleId) {
    return (
      <VehicleAnalyticsPage
        vehicleId={analyticsVehicleId}
        users={users}
        toast={toast}
        onBack={() => { setAnalyticsVehicleId(null); load(page, search); }}
      />
    );
  }
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {vehicles.map(v => (
                    <VehicleRow
                      ...
                      onAnalytics={() => setAnalyticsVehicleId(v.id)}
                      toast={toast}
                    />
                  ))}
                </div>
