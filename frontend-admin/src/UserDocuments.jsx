...
function Viewer({ doc, onClose }) {
  ...
  function apply(sc, ox, oy) {
    const clamped = sc <= 1 ? { x: 0, y: 0 } : { x: ox, y: oy };
    stateRef.current = { scale: sc, offset: clamped };
    setScale(sc); setOffset(clamped);
  }
  ...
  useEffect(() => {
    ...
    function onTouchStart(e) {
      if (e.touches.length === 2) {
        ...
        pinchRef.current = { startDist: Math.hypot(dx, dy), startScale: stateRef.current.scale, startOffset: { ...stateRef.current.offset } };
        dragRef.current = null;
      } else if (e.touches.length === 1) {
        dragRef.current = { startX: e.touches[0].clientX - stateRef.current.offset.x, startY: e.touches[0].clientY - stateRef.current.offset.y };
      }
    }
    function onTouchMove(e) {
      if (e.touches.length === 2 && pinchRef.current) {
        ...
        const dx = e.touches[0].clientX - e.touches[1].clientX;
        const dy = e.touches[0].clientY - e.touches[1].clientY;
        const sc = Math.max(1, Math.min(5, pinchRef.current.startScale * (Math.hypot(dx, dy) / pinchRef.current.startDist)));
        apply(sc, pinchRef.current.startOffset.x, pinchRef.current.startOffset.y);
      } else if (e.touches.length === 1 && dragRef.current && stateRef.current.scale > 1) {
        e.preventDefault();
        apply(stateRef.current.scale, e.touches[0].clientX - dragRef.current.startX, e.touches[0].clientY - dragRef.current.startY);
      }
    }
    function onTouchEnd(e) {
      ...
      if (e.touches.length === 0) {
        const now = Date.now();
        if (now - lastTap.current < 300) { stateRef.current.scale > 1 ? apply(1, 0, 0) : apply(2.5, 0, 0); }
        lastTap.current = now;
        dragRef.current = null;
      }
    }
  ...
  return (
    <div style={{ position:'fixed',inset:0,zIndex:9999,background:'#000',display:'flex',flexDirection:'column',overscrollBehavior:'none' }}>
      ...
      <div style={{ flex:1,display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',padding:12,touchAction:isImage?'none':'auto' }}>
        {isImage
          ? <img ref={imgRef} src={doc.url} alt={doc.label} draggable={false}
              style={{ maxWidth:'100%',maxHeight:'100%',objectFit:'contain',borderRadius:8,userSelect:'none',
                transform:`scale(${scale}) translate(${offset.x/scale}px,${offset.y/scale}px)`,
                transformOrigin:'center center',transition:pinchRef.current?'none':'transform 0.15s ease' }} />
      ...
      <div style={{ padding:'10px 16px',background:'rgba(255,255,255,0.04)',textAlign:'center',flexShrink:0 }}>
        <p style={{ fontSize:11,color:'rgba(255,255,255,0.3)' }}>{isImage ? 'Pinch to zoom · Double-tap · Tap ‹ to go back' : 'Show this screen to the authority · Tap ‹ to go back'}</p>
      </div>
    </div>
  );
}
    ...
    <>
      <div className="page-wrapper page-enter">
        ...
        <div style={{
          ...
          padding: '16px 16px 48px', position: 'relative', overflow: 'hidden',
        }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', position:'relative', zIndex:1 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <div style={{ width:38, height:38, background:'rgba(255,255,255,0.18)', borderRadius:11,
                display:'flex', alignItems:'center', justifyContent:'center', backdropFilter:'blur(4px)' }}>
                <FolderOpen size={18} color="#fff" />
              </div>
              <div>
                <p style={{ fontSize:16, fontWeight:800, color:'#fff' }}>Documents</p>
            ...
            {tab === 'my' && (
              <button onClick={() => setShowUpload(true)}
                style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px',
                  borderRadius:10, background:'rgba(255,255,255,0.18)', border:'1px solid rgba(255,255,255,0.25)',
                  color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer', backdropFilter:'blur(4px)' }}>
                <Plus size={13} /> Upload
              </button>
            )}
          </div>
        ...
        {/* Tabs — overlap header */}
        <div style={{ padding:'0 12px', marginTop:-22, position:'relative', zIndex:2, marginBottom:8 }}>
          <div style={{ display:'flex', gap:8, background:'var(--bg-card)', borderRadius:14,
            padding:5, boxShadow:'var(--shadow-md)', border:'1px solid var(--border-subtle)' }}>
            {[
              { key:'my',      icon:<User size={13} />, label:'My Docs',      count: myDocs.length },
              { key:'vehicle', icon:<Car  size={13} />, label:'Vehicle Docs', count: totalVehicleDocs },
            ].map(t => (
              <button key={t.key} onClick={() => setTab(t.key)}
                style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:6,
                  padding:'9px 0', borderRadius:10, fontSize:12, fontWeight:700, cursor:'pointer',
                  border:'none',
                  background: tab===t.key ? 'linear-gradient(135deg,#1565c0,#1976d2)' : 'transparent',
                ...
                {t.count > 0 && (
                  <span style={{ fontSize:10, fontWeight:800,
                    background: tab===t.key ? 'rgba(255,255,255,0.25)' : 'var(--bg-elevated)',
                    color: tab===t.key ? '#fff' : 'var(--text-muted)',
